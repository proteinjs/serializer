...
export type CustomSerializer = Loadable &
...
const getCustomSerializers = () =>
  SourceRepository.get().objects<CustomSerializer>('@proteinjs/serializer/CustomSerializer');
...
const getThirdPartyLibCustomSerializers = () =>
  SourceRepository.get().objects<ThirdPartyLibCustomSerializer>('@proteinjs/serializer/ThirdPartyLibCustomSerializer');
...
export class Serializer {
  ...
  private static _serialize(serializable: any) {
    ...
    if (typeof serializable === 'object' && typeof serializable.__serializerId === 'string') {
      ...
      for (const serializer of thirdPartyLibCustomSerializers) {
      ...
      if (thirdPartyLibCustomSerializer) {
        serializable = Object.assign(
          { __serializerId: thirdPartyLibCustomSerializer.id },
          thirdPartyLibCustomSerializer.serialize(serializable)
        );
      }
      serialized = {};
      Object.keys(serializable)
